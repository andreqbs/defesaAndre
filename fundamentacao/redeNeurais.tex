...
Conforme mostrado na Figura \ref{fig:neuronio_cap2}, a função sigmoide é uma função de comutação suave que tem as seguintes propriedades:
...
\addplot[domain=-1:1, red, ultra thick,samples=500] {1 / (1 + exp(-10*x))};
\addlegendentry{$f(x)=\frac{1}{1+e^{-v}}$}
\end{axis}
\end{tikzpicture}
\end{tikzpicture}
...
enlargelimits=false]
% plot the stirling-formulae
\addplot[domain=-1:1, red, ultra thick,samples=500] {(exp(2*x) - exp(-2*x)) / (exp(2*x) + exp(-2*x))};
...
diminuição do erro e, portanto, o movimento deve ser encorajado naquela direção. Este método de correção dos pesos sinápticos é denominado de Regra Delta. Ela é definida pela
expressão $\Delta \underline{w}(n) = -\eta \Delta J(w(n))$, onde $\Delta J(\underline{w}(n)) = \dfrac{J(\underline{w}(n))}{\underline{w}(n)} = \dfrac{\dfrac{1}{2}e^2(n)}{\underline{w}(n)}$ é o gradiente local da superfície de erro gerada pela função de custo $J = J(\underline{w}(n)) = \dfrac{1}{2}e^2(n)$ a ser minimizada no instante $n$.
